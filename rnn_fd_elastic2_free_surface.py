...
class Propagator2D(torch.nn.Module):
    ...
    def __parameter_den_to_Lame(self, vp, vs, rho):
        self.lambda_p = vp**2*rho - 2*vs**2*rho
        self.mu_p = vs**2*rho
        self.rho_p  = rho

        self.bx_first_rwo = 2/(self.rho_p[:,:,0,:])
        self.bx_rest_rwo = 1/(self.rho_p[:,:,1:,:])
        #print("this is the shape of bx_first_rwo and bx_first_rwo", self.bx_first_rwo.shape, self.bx_rest_rwo.shape)
        self.bx = torch.cat([self.bx_first_rwo.unsqueeze(dim=0), self.bx_rest_rwo], dim=2)

        self.bz = 1/(self.rho_p)

        self.n_1_x_first_rwo = ((2*self.mu_p*(self.lambda_p+self.mu_p))/(self.lambda_p + 2*self.mu_p))[:,:,0,:]
        self.n_1_x_rest_rwo = (self.lambda_p + 2*self.mu_p)[:,:,1:,:]
        self.n_1_x = torch.cat([self.n_1_x_first_rwo.unsqueeze(dim=0), self.n_1_x_rest_rwo], dim=2)


        self.zero_row = torch.as_tensor(torch.zeros(1, 1, 1, self.mu_p.size(3)), dtype=self.dtype).to(self.device)


        self.n_2_x_rest_rwo = self.lambda_p[:,:,1:,:]
        self.n_2_x = torch.cat([self.zero_row, self.n_2_x_rest_rwo], dim=2)

        self.n_1_z_rest_rwo = self.lambda_p[:,:,1:,:]
        self.n_1_z = torch.cat([self.zero_row, self.n_1_z_rest_rwo], dim=2)

        self.n_2_z_rest_rwo = (self.lambda_p + 2*self.mu_p)[:,:,1:,:]
        self.n_2_z = torch.cat([self.zero_row, self.n_2_z_rest_rwo], dim=2)

        return self.bx, self.bz, self.n_1_x, self.n_2_x, self.n_2_x, self.n_2_z
    ...
    def ___step_rnncell(self, source, vx, vz, vx_x, vx_z, vz_x, vz_z,txx, tzz, txz, txx_x, txx_z, tzz_x, tzz_z, txz_x, txz_z):
        ...
        var_txx_x = F.conv2d(txx, self.kernelx2D.repeat(txx.shape[1], 1, 1, 1), padding=2, groups=txx.shape[1])*self.bx
        
        var_txz_z = F.conv2d(txz, self.kernelz2D_U.repeat(txz.shape[1], 1, 1, 1), padding=2, groups=txx.shape[1])*self.bx
        
        var_txz_x = F.conv2d(txz, self.kernelx2D_U.repeat(txz.shape[1], 1, 1, 1), padding=2, groups=txx.shape[1])*self.bz
        
        var_tzz_z = F.conv2d(tzz, self.kernelz2D.repeat(tzz.shape[1], 1, 1, 1), padding=2, groups=txx.shape[1])*self.bz
        ...
        var_vx_x = F.conv2d(vx, self.kernelx2D_U.repeat(vx.shape[1], 1, 1, 1), padding=2, groups=txx.shape[1])
        
        var_vx_z = F.conv2d(vx, self.kernelz2D.repeat(vx.shape[1], 1, 1, 1), padding=2, groups=txx.shape[1])
        
        var_vz_x = F.conv2d(vz, self.kernelx2D.repeat(vz.shape[1], 1, 1, 1), padding=2, groups=txx.shape[1])
        
        var_vz_z = F.conv2d(vz, self.kernelz2D_U.repeat(vx.shape[1], 1, 1, 1), padding=2, groups=txx.shape[1])

        txx_x = (1 - self.dt * self.ax) * txx_x + self.dt *(self.n_1_x)* var_vx_x  # the txx in x direction
        txx_z = (1 - self.dt * self.az) * txx_z + self.dt *(self.n_2_x)* var_vz_z  # the txx in x direction
        txx = txx_x + txx_z # the final txx 

        tzz_x = (1 - self.dt * self.ax) * tzz_x + self.dt * (self.n_1_z)* var_vx_x  # the tzz in x direction
        tzz_z = (1 - self.dt * self.az) * tzz_z + self.dt * (self.n_2_z)* var_vz_z  # the tzz in x direction
        tzz = tzz_x + tzz_z # the final tzz

        txz_x = (1 - self.dt * self.ax) * txz_x + self.dt *(self.mu_p)* var_vx_z  # the tzz in x direction
        txz_z = (1 - self.dt * self.az) * txz_z + self.dt * (self.mu_p) * var_vz_x  # the tzz in x direction
        txz = txz_x + txz_z # the final tzz

        return vx, vz, vx_x, vx_z, vz_x, vz_z, txx, tzz, txz,txx_x, txx_z, tzz_x, tzz_z, txz_x, txz_z, self.ax, self.az  
    ...
    segment_sources,vx,vz,vx_x,vx_z,vz_x,vz_z,txx,tzz,txz,txx_x,txx_z,tzz_x,tzz_z,txz_x,txz_z):
        ...
        self.bx, self.bz, self.n_1_x, self.n_2_x, self.n_2_x, self.n_2_z = self.__parameter_den_to_Lame(self.vp_tensor, self.vs_tensor, self.rho_tensor)
